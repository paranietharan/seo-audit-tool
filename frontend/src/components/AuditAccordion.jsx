...
export default function AuditAccordion({ audit }) {
  ...
  return (
    <div className="bg-slate-800 border border-slate-700 rounded-xl shadow-lg overflow-hidden">
      ...
      {isOpen && (
        <div className="p-6 border-t border-slate-700 bg-slate-900/30 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div className="space-y-1">
              <span className="text-xs uppercase tracking-wider text-slate-400">Target URL</span>
              <p className="text-sm font-semibold text-slate-200 break-all">{audit.url}</p>
            </div>
            <div className="space-y-1">
              <span className="text-xs uppercase tracking-wider text-slate-400">Page Title</span>
              <p className="text-sm font-semibold text-slate-200">{audit.title || '(none)'}</p>
            </div>
            <div className="space-y-1">
              <span className="text-xs uppercase tracking-wider text-slate-400">Canonical URL</span>
              <p className="text-sm font-semibold text-slate-200 break-all">{audit.canonical_url || '(none)'}</p>
            </div>
            <div className="space-y-1">
              <span className="text-xs uppercase tracking-wider text-slate-400">Meta Keywords</span>
              <p className="text-sm font-semibold text-slate-200">{audit.meta_keywords || '(none)'}</p>
            </div>
            <div className="space-y-1">
              <span className="text-xs uppercase tracking-wider text-slate-400">Robots Meta</span>
              <p className="text-sm font-semibold text-slate-200">{audit.robots_meta || '(none)'}</p>
            </div>
            <div className="space-y-1">
              <span className="text-xs uppercase tracking-wider text-slate-400">Structured Data</span>
              <p className="text-sm font-semibold text-slate-200">
                {audit.has_structured_data ? (
                  ...
                  <span className="text-slate-405">None detected</span>
                )}
              </p>
            </div>
            <div className="space-y-1">
              <span className="text-xs uppercase tracking-wider text-slate-405">Status Code</span>
              <p className="text-sm font-semibold text-slate-200">{audit.status_code}</p>
            </div>
            <div className="space-y-1">
              <span className="text-xs uppercase tracking-wider text-slate-405">Load Time</span>
              <p className="text-sm font-semibold text-slate-200">{audit.page_load_ms} ms</p>
            </div>
            <div className="space-y-1">
              <span className="text-xs uppercase tracking-wider text-slate-405">Word Count</span>
              <p className="text-sm font-semibold text-slate-200">{audit.word_count} words</p>
            </div>
          </div>
          ...
          <div>
            <h4 className="text-sm font-bold uppercase tracking-wider text-slate-400 mb-3">Headings Distribution</h4>
          ...
          <div>
            <h4 className="text-sm font-bold uppercase tracking-wider text-slate-400 mb-3">Open Graph Data</h4>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="space-y-1">
                <span className="text-xs text-slate-400 block">og:title</span>
                <p className="text-sm font-semibold text-slate-300">{audit.og_title || '(none)'}</p>
              </div>
              <div className="space-y-1">
                <span className="text-xs text-slate-400 block">og:description</span>
                <p className="text-sm font-semibold text-slate-300">{audit.og_description || '(none)'}</p>
              </div>
              <div className="space-y-1">
                <span className="text-xs text-slate-400 block">og:image</span>
          ...
          <div>
            ...
            {(!audit.images_missing_alt || audit.images_missing_alt.length === 0) ? (
              <p className="text-sm text-emerald-400">All images have alternative descriptions!</p>
            ) : (
              <div className="max-h-40 overflow-y-auto bg-slate-800 border border-slate-700/50 rounded-lg p-3 space-y-2">
                {audit.images_missing_alt.map((src, i) => (
                  <div key={i} className="text-xs text-rose-350 font-mono truncate hover:bg-slate-700 p-1 rounded transition-colors" title={src}>
                    {src}
                  </div>
                ))}
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="max-h-60 overflow-y-auto bg-slate-900 border border-slate-700/50 border-t-0 rounded-b-lg p-3 space-y-3">
                  {audit.internal_links.map((link, i) => (
                    <div key={i} className="text-xs space-y-0.5 border-b border-slate-800 pb-2 last:border-b-0">
                      <span className="text-slate-400 block font-semibold">Text: "{link.text || '(empty)'}"</span>
            ...
            <div>
              <button
                ...
                className="w-full flex items-center justify-between text-left text-sm font-bold text-slate-300 bg-slate-800 border border-slate-700 px-4 py-2.5 rounded-lg hover:bg-slate-700 transition-colors focus:outline-none"
              >
                <span>External Links ({audit.external_links?.length || 0})</span>
